...
### PM-02 — Learn launcher/configuration patterns; use product-owned authenticated HTTP transport
...
- **Classification/status:** **Learned pattern** — absolute-path/configuration guidance retained; the upstream unauthenticated stdio/SSE transport is rejected. The product-owned bootstrap uses authenticated Streamable HTTP on loopback.
- **Destination:** Buildout 11 thin authenticated MCP adapter and deployment/client configuration tooling.
- **Product contract/rationale:** configuration must keep business logic outside MCP and credentials outside client configuration. The Bootstrap emits an explicit Codex CLI registration command for loopback Streamable HTTP with `--bearer-token-env-var`; it neither rewrites Codex configuration nor stores bearer material there.
...
- **Validation plan:** local tests assert bearer/origin rejection and secret-free installation documents. Windows evidence must demonstrate user-only token storage plus Codex and a second MCP client completing the read-only connectivity probe while direct unauthenticated calls fail closed.
...
- **Preparation evidence:** the primitive gateway exposes separate typed `execute_command` and `collect_results` calls. Result cells explicitly distinguish available, missing, unsupported, and non-finite values while preserving source evidence and normalized quantities. Buildout 5 adds owner-only immutable calculation storage, comparisons, and derived overlays; native command/result parity remains Windows validation work.
...
- **Preparation update:** Buildout 4 uses a separately pinned `networkx` runtime dependency for a rebuildable local `MultiGraph` projection. No exporter code or PSDM dependency was adopted. Product records remain SQLite source of truth, preserve relationship IDs for parallel equipment, and use an owned transformer-terminal representation; exporter compatibility remains a future Windows-gated evaluation.
