...
| Development | Buildout 6 portable workflow core | The platform-independent Buildout 2 owner/primitive boundary, provisional PowerFactory 2026 adapter, Buildout 3 inventory, Buildout 4 SQLite/NetworkX graph core, Buildout 5 calculation/result overlays, and all four Buildout 6 companion specifications are prepared. Next implement the durable workflow foundation behind these provisional contracts without enabling unvalidated live behavior. |
...
| Buildout 1 / `DOM` | Immutable typed contracts, canonical JSON/digests, strict defaults, structured errors, and the deterministic bounded headline workflow are implemented. The additive provisional domain schema is `0.5.0` after admitting inventory, primitive-gateway, graph, and calculation/result DTOs. | Prepared, not accepted. B0, `DEP`, and `ID` acceptance prerequisites remain unsatisfied; item 7 stays unchecked. |
