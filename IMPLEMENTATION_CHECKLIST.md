...
| Development | Buildout 6 reconciliation core | The platform-independent Buildout 2 owner/primitive boundary, provisional PowerFactory 2026 adapter, Buildout 3 inventory, Buildout 4 SQLite/NetworkX graph core, Buildout 5 calculation/result overlays, Buildout 6 specifications, workflow persistence, portable approval persistence, and context lease/fencing persistence are prepared. Next implement write-ahead intent, observation, reconciliation, and their workflow integration without enabling unvalidated live behavior. |
...
| Buildout 1 / `DOM` | Immutable typed contracts, canonical JSON/digests, strict defaults, structured errors, and the deterministic bounded headline workflow are implemented. The additive provisional domain schema is `0.8.0` after admitting inventory, primitive-gateway, graph, calculation/result, workflow, approval, and lease DTOs. | Prepared, not accepted. B0, `DEP`, and `ID` acceptance prerequisites remain unsatisfied; item 7 stays unchecked. |
