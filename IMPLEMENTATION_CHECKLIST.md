...
| Development | Buildout 6 lease/recovery core | The platform-independent Buildout 2 owner/primitive boundary, provisional PowerFactory 2026 adapter, Buildout 3 inventory, Buildout 4 SQLite/NetworkX graph core, Buildout 5 calculation/result overlays, Buildout 6 specifications, workflow persistence, and portable approval persistence are prepared. Next implement context lease/fencing and reconciliation services without enabling unvalidated live behavior. |
...
| Buildout 1 / `DOM` | Immutable typed contracts, canonical JSON/digests, strict defaults, structured errors, and the deterministic bounded headline workflow are implemented. The additive provisional domain schema is `0.7.0` after admitting inventory, primitive-gateway, graph, calculation/result, workflow, and approval DTOs. | Prepared, not accepted. B0, `DEP`, and `ID` acceptance prerequisites remain unsatisfied; item 7 stays unchecked. |
